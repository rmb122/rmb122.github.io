...
这种情况在 utf8 中就不存在, 因为 utf8 标准已经规定了在多字节编码的情况下, 除了第一位的字节在二进制下都是以 1 开头的, 也就是大于 0x7f, 保证了只要出现了二进制下 0 开头的字节, 一定是新的字符.  
...
另外, 也可以通过流量来证明, 客户端是并没有多余的步骤, 是按照 encode 后结果直接发送的  
...
可以看到出现了两次 `你好` 一次 `浣犲ソ`, 可以确定不管客户端的编码是如何, mysql 仍然是按照创建表时所指定的 gbk 存储的.  
...
$ret = $conn->query("SELECT * FROM test_gbk WHERE id='$id';");
var_dump($ret->fetch_all());
```
如果没有使用 `mysqli::set_charset` 就算是 `mysqli::real_escape_string` 也是无法避免注入的, `addslashes` 就更不用说了. 但是需要注意的是 `$conn->query("SET NAMES 'gbk'");` 也是必须的, 否则在 mysql 侧, 会认为 `"\xdf"` 会认为是一个字符而不是 `"\xdf\\"`, 导致反斜杠又生效了.